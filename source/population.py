...
class Population:
    ...
    def reproduce(self, env, stopped_NC):
        ...
        if "N" in self.selection_type:
            ...
            niche_capacity = int(np.sum([niche_info["capacity"] for niche_info in env.niches.values()]))
        ...
        for niche_idx, niche_data in enumerate(for_reproduction):
            ...
            if len(niche_pop):
                ...
                for idx, agent in enumerate(agents_reproduce):
                    ...
                    if agent.genome.type == "niche-construction" or agent.genome.type == "niche-construction-positive":
