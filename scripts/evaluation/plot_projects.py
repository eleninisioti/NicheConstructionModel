...
class Plotter:
    ...
    def plot_heatmap_mean(self, log, save_dir):
        constructed = []
        trial = list(log.keys())[0]
        log = log[trial]
        for gen_idx, gen in enumerate(log["construct"]):
            niches_indexes = []
            new_constructed = []
            for key, el in gen:
                if -35 < key < -30:
                    if 90 < gen_idx <180:
                        ...
                        new_constructed.append(np.mean(el))
        ...
        constructed_array = constructed_array[::-1]
        #constructed_array = constructed_array[0,:]

        constructed_array = pd.DataFrame(constructed_array)
        plt.figure(figsize=self.fig_size)
        sns.heatmap(constructed_array, cmap='YlGnBu')
        plt.xlabel("Generations")
        plt.ylabel("Niche index")
        plt.yticks(range(len(niches_indexes))[::20], sorted(niches_indexes)[::-1][::20])

        #plt.colorbar()

        plt.savefig(save_dir + "/plots/focused_trial_" + str(trial) + ".pdf", dpi=300)
        plt.clf()
    ...
    def plot_heatmap(self, log, save_dir):
        ...
        trial = list(log.keys())[0]
        log = log[trial]
        plt.figure(figsize=(self.fig_size[0]*2, self.fig_size[0]*2))
        gen_indexes = []
        ...
        plt.xlabel("Generations")
        plt.ylabel("Niche index")
        print(list(range(len(niches_indexes)))[::2],sorted(niches_indexes)[::-1][::2] )
        print(list(range(len(niches_indexes)))[::2],sorted(niches_indexes)[::-1][::2] )
        step = 2
        step = 20
        plt.yticks(list(range(len(niches_indexes)))[::step], sorted(niches_indexes)[::-1][::step])
        #plt.xticks(list(range(len(gen_indexes)))[::10], sorted(gen_indexes)[::-1][::10])


        #plt.colorbar()

        plt.savefig(save_dir + "/plots/focused_trial_" + str(trial) + ".pdf")
        plt.savefig(save_dir + "/plots/focused_trial_" + str(trial) + ".png",dpi=300)
...
def run(project, total):
    ...
    include = ["mean", "num_agents", "sigma", "mutate", "construct"]
    #include = ["mean", "construct","climate",
    #           "competition",   "construct_sigma", "sigma", "mutate", "num_agents", "extinct"]
    #include = ["dispersal", "mean"]
            ...
            if total:
                ...
                if trial in list(log_niches_total.keys()):
                    ...
                    if not log_trial.empty:
                        ...
                        # save for debugging
                        # construct_means = log_trial["construct"].tolist()
                        # construct_vars = log_trial["construct_sigma"].tolist()
                        # with open(project + "/for_debug_" + str(trial) + ".pkl", "wb") as f:
                        #     pickle.dump([construct_means, construct_vars], f)

                        #-----------------

                    if "construct" in config["genome_type"]:
                        plotter.plot_heatmap(log_niches_trial, project)
                        #plotter.plot_heatmap_mean(log_niches_trial, project)
